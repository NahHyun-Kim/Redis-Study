...
public class MelonMapper implements IMelonMapper {
	...
	public boolean createCollection(String colNm) throws Exception {
		...
		// 컬렉션 생성 및 인덱스 생성, MongoDB 에서 데이터 가져오는 방식에 맞게 인덱스 반드시 생성하자!
		// 데이터 양이 많지 않으면 문제되지 않으나, 최소 10만건 이상 데이터 저장 시 속도가 약 10배 이상 발생함
		mongodb.createCollection(colNm).createIndex(new BasicDBObject("collect_name", 1).append("rank", 1), "rankIdx");
	...
	public List<MelonDTO> getRank(String colNm) throws Exception {
		...
		while (cursor.hasNext()) {
			...
			String collect_time = CmmUtil.nvl((String) current.get("collect_time")); //수집 시간
			String rank = CmmUtil.nvl((String) current.get("rank")); //순위
			String song = CmmUtil.nvl((String) current.get("song")); //노래 제목
	...
	public List<MelonSingerDTO> getRankForSinger(String colNm) throws Exception {
		...
		//***aggregation 사용 시 , allowDistUse(true)를 통해 디스크 메모리 사용을 허용***
		AggregationOptions options = AggregationOptions.builder().allowDiskUse(true).build();
		
		//쿼리 실행하기
		Cursor cursor = rCol.aggregate(pipeline, options);
		
		//컬렉션으로부터 전체 데이터 가져온 것들을 List형태로 저장하기 위한 변수 선언
		...
		while (cursor.hasNext()) {
			...
			/* mongodb는 방대한 데이터 양을 저장하기 때문에, default값으로 long 형태의 정수값이 저장
			 * count한 결과를 가져올 때는 int형으로 형변환하여 가져옴
			 * */
			...
			rDTO.setRank(rank); //조회된 레코드 순서대로 랭킹을 정의함
			rDTO.setSinger(singer); 
	...
	public List<MelonRankDTO> getCompareRank(String curColNm, String preColNm) throws Exception {
		...
		List<DBObject> pipeline = Arrays.asList(
                ...
                new BasicDBObject()
                        .append("$unwind", new BasicDBObject()
                                ...
                                .append("preserveNullAndEmptyArrays", true) //left 조인은 true, inner 조인(교집합)은 false
                        ), 
                new BasicDBObject()
                        .append("$project", new BasicDBObject()
                                ...
                                .append("pre_rank", "$"+ preColNm + ".rank") //이전 컬렉션의 노래 랭킹
                                .append("_id", 0)
                        )
        );
		...
		while (cursor.hasNext()) {
			...
			//몽고DB 는 project에서 사칙 연산을 제공하지 않아, 자바에서 처리
