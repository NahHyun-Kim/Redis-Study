...
public class MelonService implements IMelonService {
	...
	public int collectMelonRank() throws Exception {
		...
		// Iterator를 사용하여 멜론 Top 100의 50위까지 순위 정보를 가져오기
		Iterator<Element> rank50List = element.select("tr.lst50").iterator(); // 멜론 50위까지 차트
		...
		while (rank50List.hasNext()) {
			...
			String album = songInfo.select("div.ellipsis a").eq(3).text(); //엘밤
			
			songInfo = null;
			
			//MongoDB에 저장할 LIst 형태의 있는 DTO 데이터 저장하기
		...
		String colNm = "MelonTOP50_" + DateUtil.getDateTime("yyyyMMdd"); //생성할 컬렉션명
	...
	public List<MelonSongDTO> getSongForSinger() throws Exception {
		...
		String colNm = "MelonTOP50_20210415"; //조회할 컬렉션명
	...
	public List<MelonRankDTO> getCompareRank() throws Exception {
		...
		String curColNm = "MelonTOP50_20210429"; //현재 랭킹 컬렉션
		String preColNm = "MelonTOP50_20210415"; //이전 랭킹 컬렉션
